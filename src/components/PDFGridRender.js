...
const PDFGridRender = (props) => {
  ...
  const [state, setState] = useState(pageAttr); // -> Master state. Stores the data of all pdf pages as an array.
  const [deletes, setDeletes] = useState([]);
  const [rotate, setRotate] = useState([]);

  useEffect(() => {
    //append new incoming page data into master state
  ...
  const addImageDataToState = (state, imageData) => {
    ...
    if (
      stateArr[objIndex] !== undefined &&
      !("imageData" in stateArr[objIndex])
    ) {
  ...
  return (
    <div>
      <button
        className={"btn-console btn btn-lg btn-secondary"}
        onClick={() => console.log(mergeData(state))}
      >
        Upload (console)
      </button>
      ...
      <ReactSortable list={state} setList={setState} animation={250}>
        {state.map((doc) => (
          <div
            className={"grid-square border-1px-solid reveal"}
            key={doc.fingerprint + "-" + doc.pageNum + "grid-key"}
          >
            <div key={doc.fingerprint + "-" + doc.pageNum} className={"d-flex"}>
              <canvas
                className={"mr-0-auto"}
            ...
            <button
              ...
              onClick={() => deletePage(state, doc.key)}
            >
              Delete
            </button>
            <button
              className="w-50 btn btn-info rounded-0 "
              onClick={() => rotatePage(state, doc)}
            >
              Rotate Clockwise
            </button>
          </div>
